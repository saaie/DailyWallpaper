...
const KEY_BING = "bing"; //Added by #SAAIE
...
const Settings = new Lang.Class({
    ...
    _schemaName: "org.gnome.shell.extensions.bingbackslide",
    /**
    ...
    /**
     * #SAAIE
     * Whether download from Bing or not.
     * @return true if download is active, false otherwise.
     */
    getBing: function(){
    	return this._setting.get_boolean(KEY_BING);
    },
    
    /**
     * #SAAIE
     * Specify, whether download from Bing or not.
     * @param bool true if download is active, false otherwise.
     * @throws TypeError if "bing" is not a boolean value.
     */
    setBing: function(bing){
    	// validate:
        if (bing === undefined || bing === null || typeof bing !== "boolean"){
            throw TypeError("bing should be a boolean variable. Got: "+bing);
        }
        // Set:
        let key = KEY_BING;
        if (this._setting.is_writable(key)){
            if (this._setting.set_boolean(key, bing)){
                Gio.Settings.sync();
            } else {
                throw this._errorSet(key);
            }
