...
/*    grid_scroll.drag_dest_set(Gtk.DestDefaults.ALL, null, Gdk.DragAction.MOVE);
    ...
    //Bing switch read/write value #SAAIE
    let bing_switch = new Gtk.Switch({active: settings.getBing()});
    bing_switch.connect('notify::active', function(button) {
    	settings.setBing(button.active);
    });
    bing_switch.set_tooltip_text("Download Bing daily background images.");
    toolbar.add(bing_switch);
