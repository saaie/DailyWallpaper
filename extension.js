...
const Gio = imports.gi.Gio;//Added by #SAAIE
const GLib = imports.gi.GLib;//Added by #SAAIE
const Soup = imports.gi.Soup;//Added by #SAAIE
const Util = imports.misc.util;//Added by #SAAIE
// Import own libs:
...
const Gettext = imports.gettext.domain('bingbackslide');
...
const BackSlideEntry = new Lang.Class({
    ...
    _init: function(){
        ...
        //Download daily backgrounds from Bing website. #SAAIE
        if (settings.getBing()){
        	let directory = Gio.file_new_for_path(downloadsDir);
        	if (!directory.query_exists(null))
        		directory.make_directory(null);
        	let _httpSession=new Soup.SessionAsync();
        	Soup.Session.prototype.add_feature.call(_httpSession, new Soup.ProxyResolverDefault());
        	let url='http://www.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1&nc=1397809837851&pid=hp';
        	let message = Soup.Message.new('GET', url);
        	_httpSession.queue_message(message, function(_httpSession, message) {
                let res = JSON.parse(message.response_body.data);
                let imageUrl="http://www.bing.com"+res.images[0].url;
...
function enable() {
    ...
    Main.panel.addToStatusArea('bingbackslide', menu_entry);
    timer.begin();
}
